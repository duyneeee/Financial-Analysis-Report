...
def prepare_financial_charts(data):
    ...
    try:
        # Check if we have enough revenue and profit data
        valid_revenue_data = any(data.get('financial_data', {}).get(year, {}).get('income_statement', {}).get('revenue', 0) > 0 for year in years)
        valid_profit_data = any(data.get('financial_data', {}).get(year, {}).get('income_statement', {}).get('net_profit', 0) > 0 for year in years)
        
        if valid_revenue_data or valid_profit_data:
            # Dữ liệu doanh thu
            ...
            # Skip if all values are zero
            if sum(revenue_data) == 0 and sum(profit_data) == 0:
                data['revenue_profit_chart'] = None
            else:
                ...
                # Thêm số liệu lên biểu đồ - only add if value is not zero
    ...
    try:
        ...
        valid_ratio_data = False
        for year in years:
            year_ratios = data.get('financial_ratios', {}).get(year, {})
            if year_ratios.get('ROA', 0) > 0 or year_ratios.get('ROE', 0) > 0 or year_ratios.get('ROS', 0) > 0:
                valid_ratio_data = True
                break
        ...
        if valid_ratio_data:
            ...
            # Skip if all values are zero
            if sum(roa_data) == 0 and sum(roe_data) == 0 and sum(ros_data) == 0:
                data['ratios_chart'] = None
            else:
    ...
    try:
        ...
        valid_bs_data = False
        for year in years:
            bs_data = data.get('financial_data', {}).get(year, {}).get('balance_sheet', {})
            if bs_data.get('total_assets', 0) > 0 or bs_data.get('liabilities', 0) > 0 or bs_data.get('equity', 0) > 0:
                valid_bs_data = True
                break
        ...
        if valid_bs_data:
            ...
            # Skip if all values are zero
            if sum(assets_data) == 0 and sum(liabilities_data) == 0 and sum(equity_data) == 0:
                data['balance_sheet_chart'] = None
            else:
                x = np.arange(len(years))
                width = 0.25
    ...
    try:
        if 'company_metrics' in data and 'sector_avg' in data:
            ...
            company_roa = data['company_metrics'].get('ROA (%)', 0)
            company_roe = data['company_metrics'].get('ROE (%)', 0)
            company_ros = data['company_metrics'].get('ROS (%)', 0)
            
            sector_roa = data['sector_avg'].get('Average ROA', 0)
            sector_roe = data['sector_avg'].get('Average ROE', 0) 
            sector_ros = data['sector_avg'].get('Average ROS', 0)
            
            valid_comparison_data = (company_roa > 0 or company_roe > 0 or company_ros > 0) and \
                                   (sector_roa > 0 or sector_roe > 0 or sector_ros > 0)
                                   
            if valid_comparison_data:
                plt.figure(figsize=(10, 6))
                
                metrics = ['ROA (%)', 'ROE (%)', 'ROS (%)']
                company_values = [company_roa, company_roe, company_ros]
                sector_values = [sector_roa, sector_roe, sector_ros]
